...
func NewSingBoxInstance(config string, localTransport LocalDNSTransport) (b *BoxInstance, err error) {
	...
	ctx = box.Context(ctx,
		wordaAndroidInboundRegistry(), wordaAndroidOutboundRegistry(), wordaAndroidEndpointRegistry(),
		wordaAndroidDNSTransportRegistry(localTransport), wordaAndroidServiceRegistry(),
	)
