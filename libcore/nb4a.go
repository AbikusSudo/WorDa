...
func InitCore(process, cachePath, internalAssets, externalAssets string,
	...
	if1 WD4AInterface, if2 BoxPlatformInterface, if3 LocalDNSTransport,
) {
	defer device.DeferPanicToError("InitCore", func(err error) { log.Println(err) })
	isBgProcess = strings.HasSuffix(process, ":bg")

	neko_common.RunMode = neko_common.RunMode_WorDaForAndroid
	intfWD4A = if1
	intfBox = if2
	...
	// nekoutils
	nekoutils.Selector_OnProxySelected = intfWD4A.Selector_OnProxySelected
