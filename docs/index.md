...
| **🏛️ Классические** | Shadowsocks, VMess, VLESS, Trojan | Широкая совместимость с существующей инфраструктурой. |
...
| :--- | :--- | :--- |
| **🔴 "Нет подключения к интернету" при активном VPN** | Неверная конфигурация или блокировка на стороне провайдера. | 1. Проверьте параметры сервера и ключи. 2. Попробуйте другой протокол (например, переключитесь с Exclave на WireGuard). 3. Проверьте работу Split Tunneling (он может направлять весь трафик мимо VPN). |
| **🟡 Высокий разряд батареи** | Активная работа в фоне, особенно с некоторыми экспериментальными протоколами. | 1. В настройках Android отключите фоновую активность для WorDa. 2. Убедитесь, что приложение не переподключается в бесконечном цикле из-за плохого сигнала. |
| **🟠 Не импортируется подписка** | Неподдерживаемый формат или нерабочая ссылка. | 1. Убедитесь, что ссылка доступна и не требует авторизации. 2. Попробуйте скопировать содержимое ссылки вручную как конфигурацию sing-box. |
| **⚫ Приложение закрывается** | Недостаточно памяти или конфликт с другим ПО. | 1. Перезагрузите устройство. 2. Убедитесь, что у вас установлена последняя версия WorDa. |
