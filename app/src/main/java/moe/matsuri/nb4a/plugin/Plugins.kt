...
import io.abikussudo.sagernet.plugin.PluginManager.loadString
import io.abikussudo.sagernet.utils.PackageCache

object Plugins {
    const val AUTHORITIES_PREFIX_SEKAI_EXE = "io.abikussudo.sagernet.plugin."
    const val AUTHORITIES_PREFIX_NEKO_EXE = "moe.matsuri.exe."

    const val ACTION_NATIVE_PLUGIN = "io.abikussudo.sagernet.plugin.ACTION_NATIVE_PLUGIN"

    const val METADATA_KEY_ID = "io.abikussudo.sagernet.plugin.id"
    const val METADATA_KEY_EXECUTABLE_PATH = "io.abikussudo.sagernet.plugin.executable_path"
    ...
    private fun getExtPluginOld(pluginId: String): List<ProviderInfo> {
        ...
        val list1 = SagerNet.application.packageManager.queryIntentContentProviders(
            Intent(ACTION_NATIVE_PLUGIN, buildUri(pluginId, "io.abikussudo.sagernet")), flags
        )
